 ...
 *  array('name' => '', 'type' => 'email','desc'=>'邮箱')
 *  array('name' => '', 'type' => 'mobile','desc'=>'手机号码')
 * ```
 *
...
class Request_Var {
  ...
  protected static function formatAllType($type, $value, $rule) {
    ...
    $formatter_class = 'Common\\Libs\\Request\\Formatter_' .ucwords(strtolower($type));

   if(! class_exists($formatter_class)){
     ERR(
       L('invalid type: {$type} for rule: {$name} {$desc}', ['TYPE' => $type, 'NAME' => $rule['name'], 'DESC' => $rule['desc']])
     );
   }
