...
class Redis {
	...
	public function __construct(){
		...
		foreach ($options['host'] as $key=>$value) {
			...
			if (!isset($options['db'][$key])) {
				$options['db'][$key] = $options['db'][0];
			}
		}
	...
	public function connect($is_master = true) {
		...
		try {
			...
			if ($this->options['db'][$i]!=0) {
				$result = $this->handler->select($this->options['db'][$i]);
				if (!$result) {
					throw new \Think\Exception('Redis Error', 103);
				}
			}
		} catch ( \Exception $e ) {
