 ...
 */
(function () {
  $.fn.JsonFormater = function (options) {
    return $(this).each(function () {
      $(this).data('JsonFormater', new JsonFormater($(this), options));
    });
  };
  function JsonFormater(obj, opt) {
    this.options = $.extend({
      dom: obj.get(0),
      tabSize: 1, //缩进数量
      singleTab: "  ", //单个tab
      quoteKeys: true,//key是否用双引号包含
      imgCollapsed: "/Public/assets/global/img/Collapsed.gif",
      imgExpanded: "/Public/assets/global/img/Expanded.gif",
    ...
    this.init();
    this.doFormat(opt.json||obj.text());
    console.log(opt.json);
    if(this.options.isCollapsible&&this.options.collapse){
      this.collapseAll();
    }
  }
